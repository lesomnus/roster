...
| P5b · the flows a mailed link finishes: recovery and verification, in `account/` | **done** — `Config.Mail` delivers what roster mints; `/recover` mails a link to an address that is somebody's (202 either way), `/redeem` proves the mailbox and hands over a **password** shown once (`Vouch.Reset`, voiding what came before) rather than a session; `/verify` mints as the app for an address the session's person owns, `/confirm` stamps it and signs nobody in. `frontdoor.Door.Redeem` beside `Accept` | `account/account.go`, `frontdoor/frontdoor.go`, `account/account_test.go` |
| P5c · account screens: profile, addresses + verify, password, factors (TOTP/WebAuthn, remove), sessions, keys, recovery form | **done** — every section a Connect call to the app's origin with the person's own reference, through the store; a section whose call the person's role does not cover says which grant it waits on. WebAuthn's ceremony is the page's (`navigator.credentials.create` → the envelope `server/vouch` checks) | `ts/account/main.tsx` |
