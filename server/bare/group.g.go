	...
	if err != nil {
		if err, ok := err.(*ent.ConstraintError); ok {
			...
			if sqlgraph.IsForeignKeyConstraintError(err) {
				return nil, status.Error(codes.NotFound, "Group: referenced entity not found")
			}
		}
		return nil, err
	}
...
func (s GroupMembershipServiceServer) Add(ctx context.Context, req *rstr.GroupMembershipAddRequest) (*rstr.GroupMembership, error) {
	...
	if err != nil {
		if err, ok := err.(*ent.ConstraintError); ok {
			if sqlgraph.IsUniqueConstraintError(err) {
				return nil, status.Error(codes.AlreadyExists, "GroupMembership already exists")
			}
			if sqlgraph.IsForeignKeyConstraintError(err) {
				return nil, status.Error(codes.NotFound, "GroupMembership: referenced entity not found")
			}
		}
		return nil, err
	}
