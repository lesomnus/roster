...
| — | payday `332ed12` | **done** — the pin moved for payday's devtools work: `entities.ts` declares each entity's `service` now, `grpcx` grew a `seam` and the generated interceptors shrank to it, and `@lesomnus/payday/react/devtools` is a panel that shows any entity's rows beside what the store holds (lesomnus/payday#3). The TS package is vendored as a tarball beside grpc-dgram's until either is released, with an `overrides` entry so there is one grpc-dgram; the console mounts the panel in development builds |
