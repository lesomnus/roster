...
| — | D37 · the review | **done** — every document read against the code, every finding attacked before it was believed. The admin port served without the leaked-password corpus and recorded half its writes; a rate counted no streams and built two buckets; a suspension did not reach `Introspect`; SIGTERM was ignored; `serve` checked one plane of two. One found and not fixed: the last-way-in race, which needs a lock no generated read offers |
...
| — | F13, upstream | **done** — the record of a soft erase was filed under the actor's tenant with an empty value, because the row it recorded was already invisible to the read that records it. Every roster entity is soft-erased. `lesomnus/payday@f1f9321`, pin moved, pinned here in `cmd/erasetrail_test.go` |
