...
| **admin** | not at contoso at all — whoever runs this deployment, and the only one of the four who uses the admin console |
...
> the admin console, and the digest of that cookie is a row here on the **control
> plane** -- alice never gets one, because the admin console is the deployment's and
...
> looked like afterwards. **admin** signing in to the admin console wrote one too, in
