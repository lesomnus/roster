...
| the page draws it: the brand and the client come from the app and not from the browser, a second form is drawn from what the server said is left to prove and never from a label it sent, one wrong code goes back to the password, and the consent screen is a screen with two answers and no checkboxes | `ts/e2e/login.spec.ts` |
