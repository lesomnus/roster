...
| the key `roster login provision` mints allows exactly what its role allows, so a policy that widens one widens both — `enrol: enrolling` puts `HolderService.Add` on each, and `invited` and `expected` on neither | `TestTheProvisionedKeyAllowsWhatThePolicyAsksFor` |
