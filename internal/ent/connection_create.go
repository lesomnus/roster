...
func (_c *ConnectionCreate) sqlSave(ctx context.Context) (*Connection, error) {
	...
	if _spec.Id.Value != nil {
		...
		if !ok {
			sv = connection.ValueScanner.Id.ScanValue()
			if err := sv.Scan(_spec.Id.Value); err != nil {
				return nil, err
			}
		}
		if value, err := connection.ValueScanner.Id.FromValue(sv); err != nil {
			return nil, err
		} else {
			_node.Id = value
		}
...
func (_c *ConnectionCreate) createSpec() (*Connection, *sqlgraph.CreateSpec, error) {
	var (
		...
		func(i int, root context.Context) {
			...
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				...
				if specs[i].Id.Value != nil {
					...
					if id, err := connection.ValueScanner.Id.FromValue(sv); err != nil {
						return nil, err
					} else {
						nodes[i].Id = id
					}
