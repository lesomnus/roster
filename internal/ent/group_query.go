...
func (_q *GroupQuery) QuerySite() *SiteQuery {
...
func (_q *GroupQuery) WithSite(opts ...func(*SiteQuery)) *GroupQuery {
	query := (&SiteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withSite; query != nil {
		if err := _q.loadSite(ctx, query, nodes, nil,
			func(n *Group, e *Site) { n.Edges.Site = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadSite(ctx context.Context, query *SiteQuery, nodes []*Group, init func(*Group), assign func(*Group, *Site)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Group)
	for i := range nodes {
		fk := nodes[i].SiteId
...
func (_q *GroupQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSite != nil {
			_spec.Node.AddColumnOnce(group.FieldSiteId)
		}
	}
