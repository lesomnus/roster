...
func (_q *TeamQuery) QuerySite() *SiteQuery {
...
func (_q *TeamQuery) Ids(ctx context.Context) (ids []uuid.UUID, err error) {
	...
	var nodes []*Team
	if nodes, err = _q.Select(team.FieldId).All(ctx); err != nil {
		return nil, err
	}
...
func (_q *TeamQuery) WithSite(opts ...func(*SiteQuery)) *TeamQuery {
	query := (&SiteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
