...
func (*Link) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case link.FieldId:
			values[i] = link.ValueScanner.Id.ScanValue()
		...
		case link.FieldHolderId:
			values[i] = link.ValueScanner.HolderId.ScanValue()
		default:
...
func (_m *Link) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		switch columns[i] {
		case link.FieldId:
			if value, err := link.ValueScanner.Id.FromValue(values[i]); err != nil {
				return err
			} else {
				_m.Id = value
			}
		...
		case link.FieldHolderId:
			if value, err := link.ValueScanner.HolderId.FromValue(values[i]); err != nil {
				return err
			} else {
				_m.HolderId = value
			}
