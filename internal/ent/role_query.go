...
func (_q *RoleQuery) QuerySite() *SiteQuery {
...
func (_q *RoleQuery) Ids(ctx context.Context) (ids []uuid.UUID, err error) {
	...
	var nodes []*Role
	if nodes, err = _q.Select(role.FieldId).All(ctx); err != nil {
		return nil, err
	}
...
func (_q *RoleQuery) WithSite(opts ...func(*SiteQuery)) *RoleQuery {
	query := (&SiteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withSite; query != nil {
		if err := _q.loadSite(ctx, query, nodes, nil,
			func(n *Role, e *Site) { n.Edges.Site = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RoleQuery) loadSite(ctx context.Context, query *SiteQuery, nodes []*Role, init func(*Role), assign func(*Role, *Site)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Role)
	for i := range nodes {
		fk := nodes[i].SiteId
...
func (_q *RoleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSite != nil {
			_spec.Node.AddColumnOnce(role.FieldSiteId)
		}
	}
