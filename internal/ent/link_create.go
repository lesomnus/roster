...
func (_c *LinkCreate) sqlSave(ctx context.Context) (*Link, error) {
	...
	if _spec.Id.Value != nil {
		...
		if !ok {
			sv = link.ValueScanner.Id.ScanValue()
			if err := sv.Scan(_spec.Id.Value); err != nil {
				return nil, err
			}
		}
		if value, err := link.ValueScanner.Id.FromValue(sv); err != nil {
			return nil, err
		} else {
			_node.Id = value
		}
...
func (_c *LinkCreate) createSpec() (*Link, *sqlgraph.CreateSpec, error) {
	...
	if id, ok := _c.mutation.Id(); ok {
		_node.Id = id
		vv, err := link.ValueScanner.Id.Value(id)
		if err != nil {
			return nil, nil, err
		}
		_spec.Id.Value = vv
		...
		func(i int, root context.Context) {
			...
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				...
				if specs[i].Id.Value != nil {
					...
					if id, err := link.ValueScanner.Id.FromValue(sv); err != nil {
						return nil, err
					} else {
						nodes[i].Id = id
					}
