...
func (_q *EmailQuery) QueryHolder() *HolderQuery {
...
// QueryVouchedBy chains the current query on the "vouched_by" edge.
func (_q *EmailQuery) QueryVouchedBy() *IdentityQuery {
...
// All executes the query and returns a list of Email entities.
...
func (_q *EmailQuery) Ids(ctx context.Context) (ids []uuid.UUID, err error) {
	...
	var nodes []*Email
	if nodes, err = _q.Select(email.FieldId).All(ctx); err != nil {
		return nil, err
	}
...
func (_q *EmailQuery) Clone() *EmailQuery {
	...
	return &EmailQuery{
		...
		withVouchedBy: _q.withVouchedBy.Clone(),
...
func (_q *EmailQuery) WithHolder(opts ...func(*HolderQuery)) *EmailQuery {
	query := (&HolderClient{config: _q.config}).Query()
...
// WithVouchedBy tells the query-builder to eager-load the nodes that are connected to
// the "vouched_by" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *EmailQuery) WithVouchedBy(opts ...func(*IdentityQuery)) *EmailQuery {
	query := (&IdentityClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withVouchedBy = query
	return _q
}
...
func (_q *EmailQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Email, error) {
	...
	if query := _q.withHolder; query != nil {
		if err := _q.loadHolder(ctx, query, nodes, nil,
			func(n *Email, e *Holder) { n.Edges.Holder = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withVouchedBy; query != nil {
		if err := _q.loadVouchedBy(ctx, query, nodes, nil,
			func(n *Email, e *Identity) { n.Edges.VouchedBy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EmailQuery) loadHolder(ctx context.Context, query *HolderQuery, nodes []*Email, init func(*Email), assign func(*Email, *Holder)) error {
...
func (_q *EmailQuery) loadVouchedBy(ctx context.Context, query *IdentityQuery, nodes []*Email, init func(*Email), assign func(*Email, *Identity)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Email)
	for i := range nodes {
