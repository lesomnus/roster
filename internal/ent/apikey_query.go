...
func (_q *ApiKeyQuery) QueryHolder() *HolderQuery {
...
// All executes the query and returns a list of ApiKey entities.
...
func (_q *ApiKeyQuery) Ids(ctx context.Context) (ids []uuid.UUID, err error) {
	...
	var nodes []*ApiKey
	if nodes, err = _q.Select(apikey.FieldId).All(ctx); err != nil {
		return nil, err
	}
...
func (_q *ApiKeyQuery) WithHolder(opts ...func(*HolderQuery)) *ApiKeyQuery {
	query := (&HolderClient{config: _q.config}).Query()
...
func (_q *ApiKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ApiKey, error) {
	...
	if query := _q.withHolder; query != nil {
		if err := _q.loadHolder(ctx, query, nodes, nil,
			func(n *ApiKey, e *Holder) { n.Edges.Holder = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ApiKeyQuery) loadHolder(ctx context.Context, query *HolderQuery, nodes []*ApiKey, init func(*ApiKey), assign func(*ApiKey, *Holder)) error {
...
func (_q *ApiKeyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withHolder != nil {
			_spec.Node.AddColumnOnce(apikey.FieldHolderId)
		}
	}
